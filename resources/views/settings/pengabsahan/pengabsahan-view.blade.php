                                ...
                                <tbody class="list">
                                    ...
                                    @foreach ($pengabsahan as $item)
                                        <tr>
                                            ...
                                            <td class="nama text-center text-break align-middle">{{ $item->user->name }}
                                            </td>
                                            <td class="foto text-center align-middle"><img
                                                    src="{{ asset('ttd') }}/{{ $item->ttd }}" width="140"
                                                    alt="">
                                            </td>
                                        ...
                                        <div class="modal fade" id="authentication-{{ $item->token_pengabsahan }}"
                                            tabindex="-1" role="dialog" aria-labelledby="authentication-modal-label"
                                            aria-hidden="true">
                                            <div class="modal-dialog mt-6" role="document">
                                                <div class="modal-content border-0">
                                                    ...
                                                    <div class="modal-body py-4 px-5">
                                                        ...
                                                        <form action="/settings/pengabsahan/{{ $item->token_pengabsahan }}"
                                                            method="POST" novalidate enctype="multipart/form-data">
                                                            @csrf
                                                            <div class="mb-3">
                                                                <label class="form-label"
                                                                    for="modal-auth-name">Verifikator</label>
                                                                <select name="verifikator" id="modal-auth-name"
                                                                    class="form-select">
                                                                    @foreach ($user as $unit)
                                                                        <option value="{{ $unit->id }}" disabled
                                                                            {{ $item->user_id == $unit->id ? 'selected' : '' }}>
                                                                            {{ $unit->name }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                            <div class="hero">
                                                                <label for="input-file{{ $item->token_pengabsahan }}"
                                                                    id="drop-area"
                                                                    class="drop-area{{ $item->token_pengabsahan }}">
                                                                    <input type="file" accept="image/*"
                                                                        id="input-file{{ $item->token_pengabsahan }}"
                                                                        name="ttd" hidden>
                                                                    <div id="image-view"
                                                                        class=" image-view{{ $item->token_pengabsahan }} text-center pt-7"
                                                                        style="background-image: url('/ttd/{{ $item->ttd }}')">
                                                                    </div>
                                                                </label>
                                                                @if ($errors->has('ttd') && session()->get('token_pengabsahan') == $item->token_pengabsahan)
                                                                    <small
                                                                        class="text-danger">{{ $errors->first('ttd') }}</small>
                                                                @endif
                                                            </div>
                                    ...
                                    <select name="verifikator" id="modal-auth-name" class="form-select">
                                        ...
                                        @foreach ($user as $unit)
                                            <option value="{{ $unit->id }}"
                                                {{ in_array($unit->id, $selectedVerifikator) ? 'disabled' : '' }}>
                                                {{ $unit->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('verifikator')
            ...
            <script>
                window.onload = function() {
                    var myModal = new bootstrap.Modal(document.getElementById(
                        'authentication-{{ $item->token_pengabsahan }}'));
                    myModal.show();
                };
            </script>
        @endif
